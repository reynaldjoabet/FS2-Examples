...
val chunk = fs2.Stream.unfoldChunk(0)(i =>
  if (i < 5) Some(Chunk.seq(List.fill(i)(i)) -> (i + 1)) else None
)
chunk.toList
...
fs2.Stream
  ...
  .map(letter =>
    fs2.Stream.emits[IO, Int](1 to 3).map(index => s"$letter$index")
  )
  .parJoin(5)
...
Stream
  .unfoldLoopEval[IO, Int, Int](0)(s =>
    if (s > 20) IO((s, None)) else IO(s, Some(s + 1))
  )
  .compile
  .toList
  .unsafeRunSync()
...
val s3 = s1.merge(s2)
val s4 = s3.evalTap(IO.println).map(x => { println(x); x })
