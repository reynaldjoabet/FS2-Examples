...
val semStream = Stream.eval[IO, Semaphore[IO]](Semaphore(8)).flatMap { sem =>
  Stream.emits(List(1, 2, 3, 4, 5)).covary[IO].parEvalMapUnordered(2) { i =>
    sem.permit.use { _ =>
...
Stream(1, 2, 3, 4, 5, 9, 0, 0, 0, 877, 7, 90, 5, 789, 87).covary[IO].parEvalMapUnordered(6)(i =>
  IO(println(i))
